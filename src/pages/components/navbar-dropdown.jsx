...
import { FaBell, FaBookmark, FaDollarSign, FaFeatherAlt, FaHeart, FaPlus, FaQuestion, FaUser, FaUserFriends } from "react-icons/fa";
import { MdSensorDoor } from "react-icons/md";

/** @param {{ size: "sm" | "md" | "lg"; }} params */
export default function NavbarDropdown({ size = "lg" }) {
  const session = useSession();
  const admins = ["kizilkartalhd@gmail.com"];

  const { isLoading, data } = SWR("/api/users");
  const user = isLoading ? {} : data.users.find((val) => val.email === session.data.user?.email);
        ...
        <DropdownMenu variant="flat">
          ...
          {admins.includes(session.data.user.email) && (
            <DropdownSection showDivider={true}>
              ...
              <DropdownItem startContent={<FaPlus />}> Yeni Seri </DropdownItem>
              <DropdownItem startContent={<FaPlus />}> Yeni Bölüm </DropdownItem>
              <DropdownItem startContent={<FaPlus />}> <Link href={"/notifications/new"}> Yeni Bildirim </Link> </DropdownItem>
            </DropdownSection>
          )}
